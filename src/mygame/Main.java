            ...
            //Choose Server/CLient
               // SettingsGUI settings = new SettingsGUI();
                //settings.inputSettings();
                 ...
                 if(isServer==1){
                   serverServer = ServerAndClientManager.setUpLocalServer(serverServer, playerThings, otherPlayer);
                   serverServer.startServer();
                 }  else if(isServer==2){
                      //local Client
                    clientNorth = ServerAndClientManager.setUpLocalClient(clientNorth,playerThings,otherPlayer);
                    clientNorth.connect();
                 } else if(isServer == 3){
                     //JMS for UPDAte
                     //erste idee (jeder server eine queue)
                     //zweite idee (1* server mit viele files)
                     //changes northBridge,south anstatt tcp clients

                     //ServerServerOnline
                    // serverServer = ServerAndClientManager.setUpOnlineServerServer(serverServer, playerThings, otherPlayer);
                    // serverServer.startServer();
                     
                     //ClientServerOnline
                     threadServerTCP = ServerAndClientManager.setUpOnlineClientServer(threadServerTCP,playerThings.getPhysicsLocation(),otherPlayer.getPhysicsLocation());
                     threadServerTCP.startServer();
                     
                     //connect Clients
                   //  ServerAndClientManager.setUpServerBridges(playerThings, otherPlayer);
                 } else if(isServer == 4){
                     //8 jms queues oder 16
                     playerClient = new EchoClient[4];
                     //bufferplayerClient[16] get content
                     //testClientOnline
                      playerClient = ServerAndClientManager.setUpOnlineClient(playerClient, playerThings, otherPlayer);   
                      for(int i = 0;i<8;i++){
                          playerClient[i].connect();
                      }
                 }
